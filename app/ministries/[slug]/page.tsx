...
export default function MinistryDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="py-12 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto space-y-8">
      <Link href="/ministries" className="inline-flex items-center gap-1.5 text-sm font-bold text-church-maroon hover:text-church-gold">
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-md space-y-6">
        <div className="border-b border-slate-200 pb-4 space-y-2">
          <span className="text-xs font-bold text-church-gold uppercase tracking-wider block">{ministry.category}</span>
          <h1 className="font-serif font-bold text-3xl sm:text-4xl text-church-maroon">
            {ministry.name}
          </h1>
        ...
        <div className="bg-amber-50 p-4 rounded-xl border border-amber-200 text-xs text-amber-900 flex items-start gap-2">
          ...
          <span>
            For meeting times, volunteer participation, or questions regarding the {ministry.name}, please contact the church office or submit a contact inquiry.
          </span>
        </div>
      </div>
    </div>
